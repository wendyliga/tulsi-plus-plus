...
class PBXTargetGeneratorTests: XCTestCase {
  ...
  override func setUp() {
    super.setUp()
    project = PBXProject(name: "TestProject", useRealGroupsPath: false)
...
class PBXTargetGeneratorTestsWithFiles: XCTestCase {
  ...
  private func validateIndexerTarget(
    ...
    productName customProductName: String? = nil,
    ...
    // Tulsi++ change PRODUCT_MODULE_NAME and PRODUCT_NAME with modulemap name so auto complete could work on xcode 13.
    // we can take any target, as all targets will have same PRODUCT_NAME
    if let customProductName = customProductName {
      expectedBuildSettings["PRODUCT_NAME"] = customProductName
    } else {
      expectedBuildSettings["PRODUCT_NAME"] = targets.first!.value.productName!
    }
